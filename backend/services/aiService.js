...
class AIService {
  ...
  async generate(type, input) {
    try {
      ...
      console.warn('Groq generation failed, falling back to OpenAI:', error.message);
      try {
        ...
        console.error('Both Groq and OpenAI failed:', fallbackError);
        throw new Error(`AI generation failed: ${error.message} (fallback also failed: ${fallbackError.message})`);
      }
    }
  }
